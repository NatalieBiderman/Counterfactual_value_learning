---
title: "create_plots_and_tables_from_source_data"
...
# because of a bug in the package that converts columns that incluse na values in it to boolean, we need to convert the relevant tables into text. we then change the values back to numeric
...
# fix an error in read_excel function
...
Note that while trying to present the figures, you might get the error: "Error in grid.Call(C_textBounds, as.graphicsAnnot(x$label), x$x, x$y,  : 
polygon edge not found". In such a case, you can simply run the code until the figure is shown. 
